...
export const FAQ: FaqItem[] = [
  ...
  },
  {
    ...
    a: "Issues collects receipts that need your attention, each with a reason chip: OCR failed (couldn't read the receipt), no amount extracted, possible duplicate, edited values differ from what was read off the receipt, over the category spending limit, or the date doesn't match the day the photo was taken. Fix the data or acknowledge the flag and the receipt drops out of Issues.",
  ...
  {
    q: "What is the Date check pill, and why is a date flagged?",
    a: "Date check compares the date read off a receipt against the day the photo was actually taken. Most till receipts print only a day and month, so the reader has to guess the month and year and occasionally gets it wrong — a September receipt filed as August lands in a report you may already have sent, and gets missed completely. A red warning means the two dates fall in different calendar months; that is always flagged, however few days apart, because it changes which monthly report the receipt belongs to. A small amber dot means the same month but the photo was taken more than three days after the receipt date, which is usually just a late upload. Open the receipt: the banner shows both dates side by side and offers the most likely correction, keeping the day printed on the receipt and taking the month from the photo. If the date really is right, press dismiss. The Date check list deliberately ignores your date filter, since a receipt that jumped months is outside whatever window you are looking at. Manually entered receipts are never flagged, as you typed the date yourself. Older receipts have no photo timestamp stored, so for those the check falls back to when the file was uploaded and only flags the pattern this reader actually gets wrong: the right day of the month with the month or year slipped. That keeps a receipt you simply uploaded weeks late from being flagged as an error.",
    q_fr: "À quoi sert la pastille Vérif. dates et pourquoi une date est-elle signalée ?",
    a_fr: "Vérif. dates compare la date lue sur un reçu au jour où la photo a été prise. La plupart des tickets n'impriment qu'un jour et un mois : le mois ou l'année peuvent donc être mal devinés, et un reçu de septembre classé en août atterrit dans un rapport déjà envoyé et passe inaperçu. Un avertissement rouge signifie que les deux dates tombent dans des mois différents ; c'est toujours signalé, même à un jour d'écart, car cela change le rapport mensuel auquel le reçu appartient. Un petit point orange signifie le même mois mais une photo prise plus de trois jours après la date du reçu, ce qui n'est en général qu'un envoi tardif. Ouvrez le reçu : la bannière affiche les deux dates côte à côte et propose la correction la plus probable, en conservant le jour imprimé sur le reçu et en prenant le mois de la photo. Si la date est correcte, touchez Ignorer. La liste Vérif. dates ignore volontairement votre filtre de dates, puisqu'un reçu qui a changé de mois est hors de la période affichée. Les reçus saisis manuellement ne sont jamais signalés, la date ayant été tapée par vous. Les reçus plus anciens n'ont pas d'horodatage de photo : la vérification se rabat alors sur la date d'envoi et ne signale que le défaut réellement commis par le lecteur — le bon jour du mois avec le mois ou l'année décalés. Un reçu simplement envoyé avec des semaines de retard n'est donc pas signalé comme une erreur.",
    q_pt: "Para que serve o indicador Verif. datas e porque é que uma data é sinalizada?",
    a_pt: "Verif. datas compara a data lida num recibo com o dia em que a foto foi tirada. A maioria dos talões imprime apenas dia e mês, pelo que o mês ou o ano podem ser adivinhados erradamente — um recibo de setembro arquivado em agosto vai parar a um relatório que já pode ter sido enviado e passa despercebido. Um aviso vermelho significa que as duas datas caem em meses diferentes; isso é sempre sinalizado, mesmo com um dia de diferença, porque muda o relatório mensal a que o recibo pertence. Um pequeno ponto âmbar significa o mesmo mês mas uma foto tirada mais de três dias depois da data do recibo, o que normalmente é só um envio tardio. Abra o recibo: a faixa mostra as duas datas lado a lado e propõe a correção mais provável, mantendo o dia impresso no recibo e usando o mês da foto. Se a data estiver certa, toque em Dispensar. A lista Verif. datas ignora deliberadamente o seu filtro de datas, já que um recibo que mudou de mês está fora da janela que está a ver. Recibos introduzidos manualmente nunca são sinalizados, pois a data foi escrita por si. Os recibos mais antigos não têm data da foto guardada: nesses casos a verificação recorre à data de carregamento e só sinaliza o erro que o leitor realmente comete — o dia do mês certo com o mês ou o ano trocados. Assim, um recibo simplesmente carregado semanas depois não é sinalizado como erro.",
    keywords: "date check wrong month year photo taken exif backdated mismatch missed report vérif dates mois verif datas mês",
  },
  {
